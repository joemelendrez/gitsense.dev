// src/components/tools/MarkdownRenderer.tsx - Pure Component
...
import { copyRichText, copyForContext } from '../../lib/rich-text-copy';
...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({
  ...
  const handleCopyRichText = async () => {
    ...
    await copyRichText(markdown, `${fileName.replace('.md', '')} - Formatted`);
  };

  const handleCopyForContext = async (context: 'slack' | 'discord' | 'notion' | 'markdown' | 'email') => {
    if (!markdown.trim()) return;
    setShowCopyOptions(false);
    await copyForContext(markdown, context, fileName.replace('.md', ''));
  };
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-8">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Smart Copy Button with Rich Text Options */}
            <div className="relative">
              <div className="flex">
                <Button
                  ...
                  className="border-green-200 text-green-600 hover:bg-green-50 rounded-r-none border-r-0"
                >
                ...
                <Button
                  ...
                  size="sm"
                  disabled={!markdown.trim()}
                  className="border-green-200 text-green-600 hover:bg-green-50 rounded-l-none px-2"
                >
              ...
              {showCopyOptions && (
                <div className="absolute top-full right-0 mt-1 w-72 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50">
                  <div className="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">
                    Copy for specific platforms
                  </div>
                  {[
                    { 
                      key: 'rich', 
                      label: '🎨 Rich Text', 
                      desc: 'Full formatting for Word, Notion, Google Docs',
                      action: handleCopyRichText
                    },
                    { 
                      key: 'notion', 
                      label: '📝 Notion', 
                      desc: 'Optimized for Notion pages',
                      action: () => handleCopyForContext('notion')
                    },
                    { 
                      key: 'slack', 
                      label: '💬 Slack', 
                      desc: 'Slack-compatible markdown format',
                      action: () => handleCopyForContext('slack')
                    },
                    { 
                      key: 'discord', 
                      label: '🎮 Discord', 
                      desc: 'Discord markdown formatting',
                      action: () => handleCopyForContext('discord')
                    },
                    { 
                      key: 'email', 
                      label: '📧 Email', 
                      desc: 'HTML email with professional styling',
                      action: () => handleCopyForContext('email')
                    },
                    { 
                      key: 'markdown', 
                      label: '📄 Clean Markdown', 
                      desc: 'Pure markdown for GitHub, docs',
                      action: () => handleCopyForContext('markdown')
                    },
                    { 
                      key: 'plain', 
                      label: '📋 Plain Text', 
                      desc: 'Simple text without formatting',
                    ...
                    <button
                      ...
                      className="flex items-start w-full px-3 py-2 text-left hover:bg-gray-50 transition-colors"
                    >
...
Perfect for AI conversations and documentation!"
      ...
      {/* Click outside to close copy options */}
      {showCopyOptions && (
        <div 
