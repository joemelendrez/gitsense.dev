...
export const CodeSummarizer: React.FC = () => {
  ...
  const summarizeCode = async (data: SummaryForm) => {
    ...
    try {
      ...
      const structuralSummary = await analyzeCodeStructure(data.code, data.language)
  ...
  const assessComplexity = (code: string): string => {
    const complexityIndicators = (code.match(/(if|for|while|switch|try|catch)/g) || []).length
...
This code appears to be a ${detectLanguage(codeValue)} module focused on data processing and user interaction. 
          ...
          <div className="space-y-4">
            {summary ? (
              <>
                ...
                <div className="flex space-x-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => copyToClipboard(summary.aiEnhanced || summary.structural)}
                  >
              ...
              <div className="bg-gray-50 rounded-md p-8 text-center text-gray-500">
                ...
                <p className="text-sm mt-2">Perfect for AI context and documentation</p>
              </div>
            )}
          </div>
        </div>
